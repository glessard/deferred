...
open class Deferred<Value>
{
  ...
  deinit
  {
    let s = deferredState.load(order: .acquire)
  ...
  func beginExecution()
  {
    var current = deferredState.load(order: .relaxed)
    repeat {
      if current & .executing != 0
      { // execution state has already been marked as begun
        return
      }
    } while !deferredState.loadCAS(current: &current, future: current | .executing, type: .weak, orderSwap: .acqrel, orderLoad: .acquire)
  }
  ...
  {
    let originalState = deferredState.load(order: .relaxed).state
    guard originalState != .resolving else { return false }

    let updatedState = deferredState.bitwiseOr(.resolving, order: .acqrel).state
    ...
    let state = deferredState.swap(.resolved, order: .acqrel)
  ...
  {
    var state = deferredState.load(order: .acquire)
    if !state.isResolved
    {
      ...
      repeat {
        ...
        if deferredState.loadCAS(current: &state, future: newState, type: .weak, orderSwap: .release, orderLoad: .relaxed)
  ...
  open func notify(queue: DispatchQueue? = nil, boostQoS: Bool = true, handler: @escaping (_ result: Result<Value, Error>) -> Void)
  {
    ...
    if !state.isResolved
    {
      ...
      waiter.deinitialize(count: 1)
      waiter.deallocate()
      _ = deferredState.load(order: .acquire)
    }
  ...
  public var state: DeferredState {
    let state = deferredState.load(order: .acquire)
  ...
  public var isResolved: Bool {
    return deferredState.load(order: .relaxed).isResolved
  }
  ...
  public var result: Result<Value, Error> {
    if deferredState.load(order: .acquire).isResolved == false
    {
  ...
  {
    if deferredState.load(order: .acquire).isResolved
    {
      return resolved
    }
    return nil
