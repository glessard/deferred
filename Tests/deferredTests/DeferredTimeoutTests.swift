...
class DeferredTimeoutTests: XCTestCase
{
  func testTimeout1()
  {
    let t1 = Deferred<Int, TestError>(resolve: { _ in }).timeout(.seconds(-1), reason: "a")
    ...
    let t3 = Deferred<Int, Error>(resolve: { _ in }).timeout(seconds: 0.01, reason: "b")
    XCTAssertEqual(t3.error, Cancellation.timedOut("b"))
  }

  func testTimeout2()
  {
    let t1 = Deferred<Int, Cancellation>(resolve: { _ in }).timeout(.seconds(-1), reason: "a")
    XCTAssertEqual(t1.error, .timedOut("a"))

    let t2 = Deferred<Int, Cancellation>(value: 0).timeout(after: .distantFuture)
    XCTAssertNil(t2.error)

    let t3 = Deferred<Int, Cancellation>(resolve: { _ in }).timeout(seconds: 0.01, reason: "b")
    XCTAssertEqual(t3.error, .timedOut("b"))
  }

  func testTimeout3()
  {
    let t1 = Deferred<Int, Never>(resolve: { _ in }).timeout(.seconds(-1), reason: "a")
    XCTAssertEqual(t1.error, .timedOut("a"))

    let t2 = Deferred<Int, Never>(value: 0).timeout(after: .distantFuture)
    XCTAssertNil(t2.error)

    let t3 = Deferred<Int, Never>(resolve: { _ in }).timeout(seconds: 0.01, reason: "b")
    XCTAssertEqual(t3.error, .timedOut("b"))
  }
}
